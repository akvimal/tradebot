...
export class OrderService {
    ...
    async updateOrder(orderId:number,qty:number,price:number,status:string,correlationId:string,comments?:string){
        let sql = `update client_orders set entry_qty = ${qty}, entry_price = ${price}, 
            status = '${status}', `;
        if(comments) sql += `comments = '${comments}', `;
        sql += `correlation_id = '${correlationId}' where broker_order_id = '${orderId}'`;
